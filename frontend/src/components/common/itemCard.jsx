...
const ItemCard = (props) => {
  ...
  useEffect(() => {
    function getItems() {
      axios.get('http://localhost:4001/api/itemmanagement').then((res) => {
  ...
  return (
    ...
    {items.map((item, index) => (
        ...
        <div className="mt-4 px-5 pb-5">
          <a href="#">
            <h5 className="text-md tracking-tight text-slate-900">{item.product_name}</h5>
          </a>
          <div className="mt-2 mb-5 flex items-center justify-between">
            <p>
              <span className="text-lg font-bold text-slate-900">{item.product_price}</span>
              {/* {OFFER} */}
