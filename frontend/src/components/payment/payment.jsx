...
function Payment() {
  ...
  function sendData(event){
    ...
    axios.post('http://localhost:8004/api/v1/deliveries', newDelivery).then(() => {
      alert('Delivery added Successfull');
      console.log(newDelivery);
    }).catch((err) => {
      alert('Error: delivery not adeed');
      console.log(err);
    })

    axios.post('http://localhost:8003/api/v1/payments', newPayment).then(() => {
      alert('Payment Successfull');
      console.log(newPayment);
    }).catch((err) => {
      alert('Error: payment not adeed');
      console.log(err);
    })
    ...
    <>
      <div className='calculation'>
        <form className='calculation_form'>
          ...
          <label>
            <input type="radio" name="delivery" value="dhl" checked={deliveryOption === "dhl"} onChange={handleDeliveryChange} />
            DHL
          </label>
          <label>
            <input type="radio" name="delivery" value="fedex" checked={deliveryOption === "fedex"} onChange={handleDeliveryChange} />
            FEDEX
          </label>
        </form>
