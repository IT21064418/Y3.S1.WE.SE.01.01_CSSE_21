...
function Payment() {
  ...
  const subtotal = data.reduce((accumulator, currentValue) => accumulator + currentValue.price * currentValue.quantity, 0);
  ...
  if (deliveryOption === "DHL") {
    deliveryFee = 0.05;
  } else if (deliveryOption === "DOMEX") {
    deliveryFee = 0.06;
  }

  const total = subtotal + (subtotal * deliveryFee) + shipping;
  ...
  return ( 
    <>
      ...
      <form className='calculation_form'>
        ...
        <label>
          <input type="radio" name="delivery" value="DOMEX" checked={deliveryOption === "DOMEX"} onChange={handleDeliveryChange} />
          DOMEX
        </label>
      </form>
  ...
  <form className="form">
    ...
    <button className="purchase--btn" onClick={() => alert("Payment Successful")}>Checkout</button> 
  </form>
</div>
    </div>
        </div>
